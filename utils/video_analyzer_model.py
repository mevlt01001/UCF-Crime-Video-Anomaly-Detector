...
class VideoSegmentDataset(Dataset):
    def __init__(self, video_path, analyzer, fps, width, height, augmentation=None):
        ...
        # decord VideoReader'ı sadece metadata almak için bir kere açıyoruz
        vr = VideoReader(video_path, ctx=cpu(0), width=width, height=height, num_threads=1)
        org_fps = vr.get_avg_fps()
        frame_indices = torch.arange(0, len(vr), step=org_fps / fps).long()
        total_frames = len(frame_indices)

        remain = (total_frames - analyzer.clip_size) % analyzer.stride
        if remain > 0:
            frame_indices = frame_indices[:total_frames - remain]
        total_frames = len(frame_indices)

        if total_frames <= analyzer.clip_size * analyzer.number_of_segments:
            raise VIDEO_TOO_SHORT_ERROR(video_path, total_frames, analyzer.number_of_segments)

        self.segment_size = total_frames // analyzer.number_of_segments
        self.frame_indices = frame_indices

    def __len__(self):
        return self.analyzer.number_of_segments

    def __getitem__(self, idx):
        # Multiprocessing içinde çökmeyi (deadlock) önlemek için vr her getitem çağrısında yerel olarak oluşturulur.
        vr = VideoReader(self.video_path, ctx=cpu(0), width=self.width, height=self.height, num_threads=1)
        
        start_idx = idx * self.segment_size
        end_idx = start_idx + self.segment_size
        segment_indices = self.frame_indices[start_idx:end_idx]
        
        segment_frames = vr.get_batch(segment_indices.tolist()).asnumpy()
        segment_tensor = torch.from_numpy(segment_frames)
        
        # Eski yapındaki clip generator mantığını kullanıyoruz
        segment_tensor = Video_Analyzer.clip_generator(segment_tensor, self.analyzer.clip_size, self.analyzer.stride)
        segment_tensor = segment_tensor.permute(0, 1, 4, 2, 3).contiguous()

        if self.augmentation:
            NC, CS, C, H, W = segment_tensor.shape
            segment_tensor = segment_tensor.view(-1, C, H, W)
            segment_tensor = self.augmentation(segment_tensor)
            segment_tensor = segment_tensor.view(NC, CS, C, H, W)
            
        return segment_tensor

@torch.inference_mode()
def extract_feats_worker(rank, world_size, analyzer, video_paths, save_dir, fps, save_debug_video, augmentation, num_workers=4):

    device = f"cuda:{rank}"
    torch.cuda.set_device(device)
    
    extractor = analyzer.feature_extractor.to(device)
    extractor.eval()
    
    chunk_size = math.ceil(len(video_paths) / world_size)
    start_idx = rank * chunk_size
    end_idx = min(start_idx + chunk_size, len(video_paths))
    my_videos = video_paths[start_idx:end_idx]
    ...
    skip_log_path = os.path.join(save_dir, f"skipped_videos_rank{rank}.txt")
    len_videos = len(my_videos)

    for video_idx, vp in enumerate(my_videos):
        ...
        try:
            dataset = VideoSegmentDataset(vp, analyzer, fps, W, H, augmentation)
            dataloader = DataLoader(dataset, batch_size=None, num_workers=num_workers, prefetch_factor=2 if num_workers > 0 else None)

            for segment_idx, segment in enumerate(dataloader):
                NC, CS, C, H_seg, W_seg = segment.shape
                
                if save_debug_video and debug_writer is not None:
                    seg_np = segment.permute(0, 1, 3, 4, 2).numpy()
                ...
                print(f"[GPU {rank}] Video: {video_idx + 1:04d}/{len_videos} [{os.path.basename(vp)}] | "
                      f"Segment: {segment_idx + 1:03d}/{analyzer.number_of_segments} - shape: {segment.shape}", flush=True)
            ...
            with open(skip_log_path, "a", encoding="utf-8") as f:
                f.write(f"{vp}\t{str(e)}\n")
            print(f"\n[Skipped - GPU {rank}] {vp} -> Err: {str(e)}", flush=True)
...
def extract_feats(analyzer: Video_Analyzer,
    ...
    # Modeli CPU'da tutuyoruz, process'ler başladığında kendi GPU'larına kopyalayacaklar
    analyzer.cpu()
    
    mp.spawn(
        extract_feats_worker,
        args=(world_size, analyzer, video_paths, save_dir, fps, save_debug_video, augmentation, num_workers_per_gpu),
        nprocs=world_size,
        join=True
    )
...
@torch.inference_mode()
def extract_feats_old(analyzer: Video_Analyzer,
                  video_paths: list,
                  save_dir: str,
                  fps: int,
                  save_debug_video: bool,
